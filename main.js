class Player {
  ...
  getUserAdjustedResults() {
    ...
    for (let validGameId of validGameIdsAllTime) {
      const validGameIdExistsSearchResults = this.results.filter(result => result.id === validGameId);
      console.log(validGameIdExistsSearchResults)
      if (validGameIdExistsSearchResults.length > 0) {
        adjustedResults.push(validGameIdExistsSearchResults[0])
      } else {
        adjustedResults.push({id: validGameId, score: 'N', attempts: '', postOrder: 0})
      }
    }
  ...
  getGamesPlayedCountByWeek(weekNumber) {
    ...
    console.log(this.results.filter(result => validGameIdsByWeek.includes(result.id)));
    return this.results.filter(result => validGameIdsByWeek.includes(result.id)).length;
  }
...
const loadPlayers = () => {
  ...
  for (let key of keys) {
    ...
    console.log(player.getGamesPlayedCountByWeek(3))
    players.push(player);
  }
};
